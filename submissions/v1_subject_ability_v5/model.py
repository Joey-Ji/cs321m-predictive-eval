"""
Subject-Ability Prediction v5: Fine-tuned for MLL < -0.6

Careful tuning based on v2's success:
- Slightly less smoothing (0.3 vs 0.5)
- Slightly faster confidence (8 vs 10)
- Keep conservative calibration approach
...
LAPLACE_ALPHA = 0.3  # Between v2 (0.5) and v4 (0.2)
SHRINK_LAMBDA = 0.5
MIN_OBSERVATIONS = 2
...
def _compute_ability(observations: list[tuple[str, float]], global_mean: float = 0.0) -> float:
    """
    Compute subject ability with moderate shrinkage.
    ...
    # Moderate shrinkage
    shrinkage = 1.0 / (1.0 + total_count / 4.0)  # Between v2 (5.0) and v4 (3.0)
...
def _fit_calibration_moderate(
    ...
    """
    Moderate calibration - between v2 and v4.
    ...
    # Start moderate
    rho = 0.9  # Between v2 (0.8) and v4 (1.0)
    beta0 = _safe_logit(observed_rate)

    # Fit with moderate penalty
    if len(z_values) >= 4:  # Between v2 (5) and v4 (3)
        try:
            def objective(params):
                ...
                # Moderate penalty - between v2 (0.1) and v4 (0.01)
                nll += 0.05 * (r - 1.0) ** 2
                return nll
            ...
            result = minimize(
                ...
                x0=[0.9, beta0],
                method="L-BFGS-B",
                bounds=[(0.2, 4.0), (None, None)],  # Between v2 and v4 bounds
            )
            if result.success:
                rho, beta0 = result.x
        except Exception:
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    """
    Predict probability with fine-tuned calibration.
    ...
    try:
        ...
        # Moderate confidence buildup - between v2 (10) and v4 (5)
