...
class AdvancedFeatureExtractor:
    ...
    def extract(self, row):
        ...
        features.extend([
            bench_stat.get("avg_correct_rate", 0.5),
            bench_stat.get("item_count", 0.0),
            bench_stat.get("avg_item_length", 0.0),
            bench_stat.get("std_item_length", 0.0),
            bench_stat.get("label_entropy", 1.0),
        ])
        # Condition features (3)
        cond_stat = self.condition_stats.get(condition, self.global_condition_defaults)
        features.extend([
            cond_stat.get("avg_correct_rate", 0.5),
            cond_stat.get("item_count", 0.0),
            cond_stat.get("benchmark_count", 0.0),
        ])
        # Interaction features (2)
        interaction_key = (benchmark, condition)
        interaction_stat = self.interaction_stats.get(interaction_key, {})
        features.extend([
            interaction_stat.get("avg_correct_rate", 0.5),
            interaction_stat.get("item_count", 0.0),
        ])
        # Stage1 global features (4)
