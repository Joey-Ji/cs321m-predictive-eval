...
try:
    from src.features import REPRESENTATION_VERSION, encode_side_features, _clean_field
except ImportError:
...
class AdvancedFeatureExtractor:
    def __init__(self, benchmark_stats, condition_stats, interaction_stats, stage1_stats, feature_names=None):
        ...
        self.feature_dim = 6  # V3: reduced from 15 to 6
        self.feature_names = feature_names or []
    
    def _compute_global_defaults(self, stats_dict):
        if not stats_dict:
            return {}
        keys = set()
    ...
    def extract(self, row):
        ...
        features = []
        # V3: Only 6 features (reduced from 15)
        # 1. Benchmark avg_correct_rate
        bench_stat = self.benchmark_stats.get(benchmark, self.global_benchmark_defaults)
        features.append(bench_stat.get("avg_correct_rate", self.global_benchmark_defaults.get("avg_correct_rate", 0.5)))

        # 2. Condition avg_correct_rate
        cond_stat = self.condition_stats.get(condition, self.global_condition_defaults)
        features.append(cond_stat.get("avg_correct_rate", self.global_condition_defaults.get("avg_correct_rate", 0.5)))
        ...
        # 4. Stage1 z_mean (global item difficulty)
        ...
        # 6. Item length (log-scaled)
        features.append(np.log1p(item_len))
    ...
    def load(cls, path):
        ...
        interaction_stats = {tuple(k.split("::")): v for k, v in state["interaction_stats"].items()}
        return cls(
...
# Load advanced feature extractor if present
ADVANCED_FEATURE_EXTRACTOR = None
ADVANCED_FEATURE_MEAN = None
ADVANCED_FEATURE_STD = None
adv_extractor_path = _resolve_file("advanced_feature_extractor.json", required=False)
if adv_extractor_path and HEAD_META.get("uses_advanced_features", False):
    ADVANCED_FEATURE_EXTRACTOR = AdvancedFeatureExtractor.load(adv_extractor_path)
    ADVANCED_FEATURE_MEAN = np.array(HEAD_META["advanced_feature_mean"], dtype=np.float32)
    ADVANCED_FEATURE_STD = np.array(HEAD_META["advanced_feature_std"], dtype=np.float32)
    print(f"[v1_kfactor_advanced] Loaded advanced feature extractor with {ADVANCED_FEATURE_EXTRACTOR.feature_dim} features", flush=True)
...
ADVANCED_FEATURE_DIM = int(HEAD_META.get("advanced_feature_dim", 0))
expected_in_dim = EMBEDDING_DIM + SIDE_FEATURE_DIM + ADVANCED_FEATURE_DIM
if expected_in_dim != IN_DIM:
    raise ValueError(
        f"head_meta in_dim {IN_DIM} != embedding_dim {EMBEDDING_DIM} + side_feature_dim {SIDE_FEATURE_DIM} + advanced_feature_dim {ADVANCED_FEATURE_DIM}"
    )
...
def _item_context(
    ...
    with torch.no_grad():
        ...
        if ADVANCED_FEATURE_EXTRACTOR is not None:
            row_for_features = {
                "benchmark": benchmark,
                "condition": condition,
                "item_content": item_content
            }
            adv_features = ADVANCED_FEATURE_EXTRACTOR.extract(row_for_features)
            # Normalize
            adv_features_norm = (adv_features - ADVANCED_FEATURE_MEAN) / ADVANCED_FEATURE_STD
            adv_t = torch.from_numpy(adv_features_norm).float()
            x = torch.cat([emb, side_t, adv_t], dim=0).reshape(1, -1)
        else:
