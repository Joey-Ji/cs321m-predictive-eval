"""Fit a residual MLP IRT model on the public training responses.

The model keeps the interpretable K-factor IRT backbone:

    base_ij = S_i + U_i @ V_j + Z_j

and adds a nonlinear residual:

    logit_ij = base_ij + MLP([S_i, Z_j, U_i, V_j, U_i * V_j, |U_i - V_j|])

This is still transductive: item embeddings are learned for observed training
items only. Use it as a Stage 1 diagnostic / pseudo-target generator, not as a
cold-start model for unseen items.
"""
...
class ResidualMLPIRT(nn.Module):
    def __init__(
        self,
        n_subjects: int,
        n_items: int,
        ...
        self.subject_bias = nn.Embedding(n_subjects, 1)
    ...
    def forward(self, subjects: torch.Tensor, items: torch.Tensor) -> torch.Tensor:
        ...
        residual = self.residual_mlp(features).squeeze(-1)
...
def fit(config: FitConfig) -> dict[str, object]:
    ...
    mlp_lr = config.mlp_lr if config.mlp_lr is not None else config.lr
    initial_embedding_lr = (
        embedding_lr * config.warmup_start_factor
        if config.warmup_epochs > 0.0
        else embedding_lr
    )
    initial_mlp_lr = (
        mlp_lr * config.warmup_start_factor
        if config.warmup_epochs > 0.0
        else mlp_lr
    )
    optimizer = torch.optim.AdamW(
        [
            {
                "params": [
                    model.subject_bias.weight,
                    model.subject_u.weight,
                    model.item_v.weight,
                    model.item_z.weight,
                ],
                "lr": initial_embedding_lr,
            },
            {
                "params": model.residual_mlp.parameters(),
                "lr": initial_mlp_lr,
            },
        ],
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for batch_idx, (subjects, items, batch_labels) in enumerate(progress, start=1):
            ...
            if warmup_steps > 0 and global_step <= warmup_steps:
                ...
                embedding_step_lr = embedding_lr * (
                ...
                optimizer.param_groups[0]["lr"] = embedding_step_lr
                optimizer.param_groups[1]["lr"] = mlp_step_lr
            ...
            progress.set_postfix(
                ...
                emb_lr=optimizer.param_groups[0]["lr"],
        ...
        history_row = {
            ...
            "embedding_lr": optimizer.param_groups[0]["lr"],
            "mlp_lr": optimizer.param_groups[1]["lr"],
        }
        ...
        if monitor_loss < best_loss:
            ...
            if (
                ...
                print(
                    "early_stopping="
                    f"{config.early_stop_patience} epochs without validation improvement"
                )
                break
    ...
    summary = {
        ...
        "best_monitor_log_loss": best_loss,
        "best_monitor_mean_log_likelihood": -best_loss,
        "history": history,
        "outputs": {
...
def main() -> None:
    ...
    parser.add_argument("--out", default="stage_1/k_factor_irt/outputs/mlp_irt", type=Path)
    ...
    parser.add_argument(
        ...
        default=None,
        type=float,
        help="Learning rate for subject/item embeddings. Defaults to --lr.",
    )
    parser.add_argument(
        "--mlp-lr",
        default=None,
        type=float,
        help="Learning rate for residual MLP parameters. Defaults to --lr.",
    )
    ...
    parser.add_argument("--smoothing", default=20.0, type=float)
    parser.add_argument("--hidden-dims", default=[128, 64], type=_parse_hidden_dims)
    parser.add_argument("--dropout", default=0.1, type=float)
    parser.add_argument(
        "--residual-scale",
        default=1.0,
        type=float,
        help="Multiplier on the MLP residual added to the IRT backbone.",
    )
    parser.add_argument(
        "--early-stop-patience",
        default=4,
